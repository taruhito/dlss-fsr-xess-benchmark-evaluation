...
# ============================ CONFIG ============================
# Input / output paths
WATT_RANKINGS_CSV = Path(os.environ.get("WATT_RANKINGS_CSV", "./watt_out/watt_rankings.csv"))
OUT_DIR = Path(os.environ.get("WATT_OUT_DIR", "./watt_out"))
...
# Output columns: family, family_real_watt_mean, family_real_watt_median, modes, rank (assigned after sorting by family_real_watt_mean)
def _build_family_table(df: pd.DataFrame) -> pd.DataFrame:
    d = df.copy()
    ...
    # Rank ascending by family_real_watt_mean (lower is better)
...
def main() -> None:
    ...
    if "family_real_watt_median" in fam_df.columns:
        cols = ["rank", "family", "family_real_watt_mean", "family_real_watt_median", "modes"]
    else:
        cols = ["rank", "family", "family_real_watt_mean", "modes"]
